...
import {
  loadBirds, saveBirds, deleteBird,
  loadPairs, savePairs, deletePair,
  loadBreeding, saveBreeding, deleteBreeding,
  loadHealth, saveHealth, deleteHealth,
  loadFinance, saveFinance, deleteFinance,
  loadAlerts, saveAlerts, deleteAlert,
  generateId,
} from './store';
...
function App() {
  ...
  // ✅ تحميل البيانات من السحابة عند فتح التطبيق
  useEffect(() => {
    let isMounted = true;
  ...
  // ✅ دوال الحذف (حذف من السحابة)
  const handleDeleteBird = useCallback(async (id: string) => {
    await deleteBird(id);
    setBirds(prev => prev.filter(b => b.id !== id));
  }, []);

  const handleDeletePair = useCallback(async (id: string) => {
    await deletePair(id);
    setPairs(prev => prev.filter(p => p.id !== id));
  }, []);

  const handleDeleteBreeding = useCallback(async (id: string) => {
    await deleteBreeding(id);
    setBreeding(prev => prev.filter(b => b.id !== id));
  }, []);

  const handleDeleteHealth = useCallback(async (id: string) => {
    await deleteHealth(id);
    setHealth(prev => prev.filter(h => h.id !== id));
  }, []);

  const handleDeleteFinance = useCallback(async (id: string) => {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'birds':
        return <BirdsPage birds={birds} onUpdate={updateBirds} onDelete={handleDeleteBird} />;
      case 'pairs':
        return <PairsPage birds={birds} pairs={pairs} onUpdatePairs={updatePairs} onUpdateBirds={updateBirds} onDeletePair={handleDeletePair} />;
      case 'breeding':
      ...
      case 'finance':
        return <FinancePage finance={finance} onUpdate={updateFinance} onDelete={handleDeleteFinance} />;
      case 'alerts':
